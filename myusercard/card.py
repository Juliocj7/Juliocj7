import os, time, hashlib, requests, datetime
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    filename = 'myusercard/cache/'+hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest()+'.txt' # Create a unique filename for each user
    try:
...
if __name__ == '__main__':
    """
    JulioCj7 (SimplyTheBest), 2024-11-22
    """
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2000, 4, 20))
    ...
    #if OWNER_ID == {'id': 'MDQ6VXNlcjgxMDQ5ODU5'}:
    ...
    svg_overwrite('myusercard/dark_mode.svg', age_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1])
    svg_overwrite('myusercard/light_mode.svg', age_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1])
